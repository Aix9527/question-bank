...
export default async function AdminPapersPage(){const papers=await apiGetAdmin<Paper[]>('/api/admin/papers');return <main className="shell"><BackTop href="/admin">← 返回管理后台</BackTop><section className="hero"><h1>试卷管理</h1><p>已发布试卷不会物理删除；删除动作统一进入 archived。</p></section><AdminPaperCreateForm/><section className="list">{papers.map(p=><AdminPaperRow key={p.id} paper={p}/>)}</section></main>}
