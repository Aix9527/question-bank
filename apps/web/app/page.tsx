...
export default async function Home() {
  ...
  return <main className="shell">
    <section className="hero"><div className="row"><div><div className="eyebrow">v0.5</div><h1>专科复习在线题库</h1><p>语文、数学、英语分别建立，做题记录按账号独立保存，可反复练习与批复。</p></div><div className="identityBox"><span>{user.display_name??user.username}</span><small>{canAdmin?'管理员 / 学习':'学习账号'}</small><div className="identityActions"><Link className="ghostButton" href="/password">修改密码</Link>{!localMode&&<LogoutButton/>}</div></div></div></section>
